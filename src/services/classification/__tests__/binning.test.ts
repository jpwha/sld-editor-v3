...
import {
  ...
  function expectArraysClose(actual: number[], expected: number[], tolerance = 0.0001) {
  ...
  describe('calculateEqualIntervalBreaks', () => {
    it('creates equal interval breaks', () => {
      ...
      const breaks = calculateEqualIntervalBreaks(values, 4, stats);
    ...
    it('handles odd number of breaks', () => {
      ...
      const breaks = calculateEqualIntervalBreaks(values, 3, stats);
  ...
  describe('calculateStdDevBreaks', () => {
    it('creates breaks based on standard deviation', () => {
      ...
      const breaks = calculateStdDevBreaks(values, 5, stats);
    ...
    it('handles even number of bins', () => {
      ...
      const breaks = calculateStdDevBreaks(values, 4, stats);
  ...
  describe('calculateJenksBreaks', () => {
    it('creates natural breaks', () => {
      ...
      const breaks = calculateJenksBreaks(values, 3);
    ...
    it('handles small datasets', () => {
      const values = [1, 2, 3];
      const breaks = calculateJenksBreaks(values, 2);
      expectArraysClose(breaks, [1, 2, 3]);
    });
  });
  
  describe('calculateBreaks', () => {
    const validOptions = { method: 'equalInterval' as const, numBins: 4, roundIncrement: 0, includeMissing: false };
  
    it('handles valid input for each method', () => {
      const values = [1, 2, 3, 4, 5, 6, 7, 8];
      const methods: ClassificationMethod[] = ['equalInterval', 'quantile', 'stdDev', 'jenks'];
      methods.forEach(method => {
        const result = calculateBreaks(values, { ...validOptions, method });
    ...
    it('applies rounding increment correctly', () => {
      ...
      const result = calculateBreaks(values, { ...validOptions, roundIncrement: 1 });
      result.breaks.forEach(break_ => expect(break_ % 1).toBe(0));
    });
  
    it('filters invalid values', () => {
      const values = [1, 2, NaN, 4, -999, 6];
      const result = calculateBreaks(values, { ...validOptions, includeMissing: true });
      expect(result.breaks.length).toBe(validOptions.numBins + 1);
