...
import { calculateBreaks } from '../index';

describe('Classification Performance', () => {
  const generateLargeDataset = (size: number): number[] =>
    Array.from({ length: size }, (_, i) => Math.random() * 1000);

  it('handles large datasets efficiently - equal interval', () => {
    const largeValues = generateLargeDataset(10000);
    const start = performance.now();
    const result = calculateBreaks(largeValues, { method: 'equalInterval', numBins: 10, roundIncrement: 0, includeMissing: false });
  ...
  it('handles large datasets efficiently - quantile', () => {
    const largeValues = generateLargeDataset(10000);
    const start = performance.now();
    const result = calculateBreaks(largeValues, { method: 'quantile', numBins: 10, roundIncrement: 0, includeMissing: false });
    const duration = performance.now() - start;
  ...
  it('handles large datasets efficiently - jenks', () => {
    const largeValues = generateLargeDataset(10000);
    const start = performance.now();
    const result = calculateBreaks(largeValues, { method: 'jenks', numBins: 10, roundIncrement: 0, includeMissing: false });
    const duration = performance.now() - start;
  ...
  it('maintains accuracy with large datasets', () => {
    ...
    const result = calculateBreaks(values, { method: 'equalInterval', numBins: 10, roundIncrement: 0, includeMissing: false });
