...
import { calculateStatistics, calculateBreaks, ClassificationError } from '../index';
...
describe('Statistics Edge Cases', () => {
  it('handles extreme value ranges', () => {
    const tinyStats = calculateStatistics([0.0000001, 0.0000002, 0.0000003]);
    const hugeStats = calculateStatistics([1e10, 2e10, 3e10]);
  ...
  it('handles values with high precision', () => {
    const stats = calculateStatistics([1.23456789, 2.34567890, 3.45678901]);
    expect(stats.mean).toBeCloseTo(2.34567860, 8);
  });

  it('handles repeated values', () => {
    const stats = calculateStatistics([1, 1, 1, 1, 1]);
...
describe('Classification Edge Cases', () => {
  it('handles skewed distributions', () => {
    const result = calculateBreaks([1, 2, 3, 100, 101, 102], {
  ...
  it('handles outliers in quantile classification', () => {
    const result = calculateBreaks([1, 2, 3, 4, 5, 1000], {
      method: 'quantile',
      numBins: 3,
      roundIncrement: 0,
      includeMissing: false,
  ...
  it('handles precision with decimal rounding', () => {
    const result = calculateBreaks([1.23, 2.34, 3.45, 4.56, 5.67], {
      method: 'equalInterval',
      numBins: 4,
      roundIncrement: 0.1,
      includeMissing: false,
  ...
  it('gracefully handles numBins > unique values', () => {
    const result = calculateBreaks([1, 2, 3], {
      method: 'equalInterval',
      numBins: 10,
      roundIncrement: 0,
      includeMissing: false,
